...
public class Caterfly extends AbstractDrawableSprite implements Disposable {
...
// All player movements are keyed from this speed.
    final static float monsterBaseSpeed = 5.5f/4.5f;
...
//        loader.dispose();
    ...
    private Caterfly.e_Actions monsterStatus = Caterfly.e_Actions.e_Idle;
    ...
    IFunc play = () -> {
        switch (monsterStatus) {
            case e_Idle:
                ...
                checkForFall();
                if (monsterStatus == Caterfly.e_Actions.e_Idle) {
// Chance of breaking out of idle animation
                    if (RangedRandom.generate(1,150)<4) {
// Chance of changing direction
                        if (RangedRandom.generate(1,100)<7) { facingRight = !facingRight; }
                        if (!facingRight) {
                            if ((FrameCount.getFrameCount()&7)==1) {
                                monsterStatus = e_Actions.e_JumpLeft;
                                monsterJumpIndex = 0;
                            } else {
                                monsterStatus = Caterfly.e_Actions.e_WalkLeft;
                            }
                        } else {
                            if ((FrameCount.getFrameCount()&7)==1) {
                                ...
                                monsterStatus = Caterfly.e_Actions.e_WalkRight;
                            }
                        }
                    }
                }
                break;
            case e_Fall:
                viewableSpriteEntity.getSprite().setAnimation("jump_loop");
                if (monsterFallSpeed < monsterBaseSpeed/2) {
                    monsterFallSpeed += 0.50;
                }
                if (monsterCheckFloor()) {
                    monsterStatus = Caterfly.e_Actions.e_Idle;
                } else {
            ...
            case e_WalkLeft:
                ...
                checkForFall();
                if (monsterStatus == Caterfly.e_Actions.e_Fall){ 		// Changed from walk into fall
            ...
            case e_JumpLeft:
                facingRight=false;
                viewableSpriteEntity.getSprite().setAnimation("jump_start");
    ...
    private void startFallLeft() {
        startFall();
        monsterStatus = Caterfly.e_Actions.e_FallLeft;
    }

    private void startFallRight() {
        startFall();
        monsterStatus = Caterfly.e_Actions.e_FallRight;
    }

    private void startFall() {
        monsterFallSpeed = monsterFallStartSpeed;
        monsterStatus = Caterfly.e_Actions.e_Fall;
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public Caterfly(Vector2 worldPosition, Vector2 dimension, boolean active, boolean basePlayMethod, boolean flipped) {
        super(worldPosition, dimension, active, basePlayMethod,flipped);
        initialise(IFileName.FNCaterfly,"idle", worldPosition);
        loadSpriteData(IFileName.FNCaterfly, "idle", drawer, loader, data, reader,
