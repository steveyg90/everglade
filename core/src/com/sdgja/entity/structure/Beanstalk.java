...
import static com.sdgja.tmx.TmxRenderer.drawStructureIntoMapSkipTop;

public class Beanstalk extends Entity {
...
//    final int MAXBOUNCE = 6;        // How many tiles to bounce the drill when it hits a solid
    ...
    IFunc play = () -> {
        ...
        structureMap = getStructureMap("beanstalk");  // structure map - need to use same name as when we created thumper
        switch (action) {
            case normal:
//                drawLightning();
                break;
            case grow:
                ...
                if (growthTimer==growthSpeed*3.3){
                    growthTimer=0;
                    currentGrowth++;
                    if (currentGrowth==TmxRenderer.getStructureHeight("beanstalk") ){
                        action=act.normal;
                    }
                    TmxRenderer.growStructureIntoMap("beanstalk", MapGeneration.getBuildingMap(), x, y,currentGrowth);
                    drawStructureIntoMapSkipTop("beanstalk", MapGeneration.getBuildingMap(), x, y,16);
                }
                break;
            default:
...
//                map[x+1][y + 2] = 276 + 128;
    ...
    public Beanstalk(int x, int y, int startPower, short[][] map) {
        super("beanstalk", x, y, map);
        TmxRenderer.allocateStructureIntoMap("beanstalk", MapGeneration.getBuildingMap(), x, y);
        {
            action = act.grow;
            currentGrowth =0;
            growthTimer=0;
